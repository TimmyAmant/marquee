...
export type QuickConnectStartResult = { handle?: string; code?: string; error?: string };

/** "Use Quick Connect": the code to approve in a Jellyfin app. */
export async function startQuickConnectAction(): Promise<QuickConnectStartResult> {
  const result = await startQuickConnect(getClientIp(await headers()));
  return result.ok ? { handle: result.handle, code: result.code } : { error: result.error };
}

/** One poll of that code; on approval signs in and goes home. */
