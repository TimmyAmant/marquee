...
desktop Chrome/Edge). From 0.46 `request_not_found` (🔍): an approved
request Sonarr/Radarr hasn't found — "Couldn't find Ice Age (2002) —
requested by Susan" to the admin and trusted members (a reminder "Still
can't find …" at most once more, a week later), and "We're still looking
for Ice Age (2002)" to the requester if they chose to hear it (in the bell
only, by default). Once it's found or dismissed its alerts are marked read.
Tapping one opens `/titles/{mediaType}/{tmdbId}` and marks it read.
...
for"), `issue_updated` ("A problem I reported is fixed"); for the admin
and trusted members also `request_pending`, `request_not_found` (0.46+,
...
`request_downloading` and `request_still_looking`. The household channels are separate —
see below.
...
saved a choice before 0.46). `request_still_looking` is never listed: the
household hears the reviewers' "Couldn't find" instead. **`PUT`** — `{ "events": { "issue_updated": true } }`: only
