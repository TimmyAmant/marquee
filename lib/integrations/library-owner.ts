...
/**
 * Resolves which user's synced Plex/Jellyfin/Sonarr/Radarr data should be
 * treated as "the household library" for ownership/status lookups. In this
 * app's shared-household model only the admin connects these integrations —
 * a member has no integrations of their own, so scoping status queries to
...
export async function getLibraryOwnerUserId(userId: string): Promise<string> {
  ...
  if (plex || jellyfin || sonarr || radarr) return userId;
