...
describe("Plex PIN handles", () => {
  ...
  it("find their PIN for the purpose that started them", () => {
    const { handle } = createPlexPinHandle({ pinId: 7, clientId: "c", purpose: signIn });
  ...
  it("are bound: a sign-in handle can't link, and a link handle only works for its account", () => {
    const signInHandle = createPlexPinHandle({ pinId: 1, clientId: "c", purpose: signIn }).handle;
    expect(getPlexPin(signInHandle, { kind: "link", userId: "u1" }).status).toBe("expired");

    const linkHandle = createPlexPinHandle({ pinId: 2, clientId: "c", purpose: { kind: "link", userId: "u1" } }).handle;
  ...
  it("expire after ten minutes", () => {
    const now = 5_000_000;
    const { handle, expiresAt } = createPlexPinHandle({ pinId: 3, clientId: "c", purpose: signIn }, now);
  ...
  it("can be claimed once", () => {
    const { handle } = createPlexPinHandle({ pinId: 4, clientId: "c", purpose: signIn });
  ...
  it("aren't checked with plex.tv more than once a second", () => {
    const { handle } = createPlexPinHandle({ pinId: 5, clientId: "c", purpose: signIn });
    const lookup = getPlexPin(handle, signIn);
